    ...
    <nb-content>
        ...
        <view v-for="thread in threads" :key="thread._id">
            <nb-list-item avatar>
                <nb-left>
                    <!-- Thread User Avatar -->
                    <nb-thumbnail small :source="{uri: thread.user.avatar}" />
                </nb-left>
                <nb-body>
                    <!-- Thread User Name -->
                    ...
                    <nb-text note :numberOfLines="1">here</nb-text>
                </nb-body>
                <nb-right>
                    <!-- Thread Date -->
            ...
            <!-- Iterate Posts Here -->
            <view class="post-container" v-for="post in thread.posts" :key="post.id">
                <nb-list-item avatar>
                    <nb-left>
                        <!-- Post User Avatar -->
                        <nb-thumbnail small :source="{uri: post.user.avatar}" />
                    </nb-left>
                    <nb-body>
                        <!-- Post User Name -->
                        <nb-text>{{post.user.name}}</nb-text>
                        <!-- Post Text -->
                        <nb-text note>{{post.text}}t</nb-text>
                    </nb-body>
                    <nb-right>
...
<nb-container v-else>
    <AppMessage message="There are no threads currently created :(" msgType="warning" />
</nb-container>
</template>
